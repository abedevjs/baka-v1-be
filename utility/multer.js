...
const multerStorage = multer.diskStorage({//How we want to store our file.
    ...
    filename: (req, file, callback) => {//this is here to define the filename. this property name from multer
        ...
        callback(null, `${req.user.nama}-${Date.now()}-${file.originalname}`);
            //first argument is error if error, and if valid is null
...
const multerFilter = (req, file, callback) => {//How to filter the upload file
    ...
    if(mimeTypes.includes(file.mimetype)) {
        ...
        callback(new AppError('Tipe file yang boleh di upload adalah .pdf, .png, .jpeg, dan .jpg dengan maksimum 5mb saja Kak 😢', 403))
    };
};
