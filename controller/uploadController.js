...
exports.updateUploadDokumen = catchAsync(async(req, res, next) => { //* www.nama.com/upload/:id?
    ...
    if(!req.params.id) {
        ...
        const newDokumen = await User.findByIdAndUpdate(currentUser, {
            $push: {
        ...
        if(!newDokumen) return next(new AppError('Kesalahan dalam upload Dokumen kak, mohon coba beberapa saat lagi 😢', 401))
    ...
    //! WITH req.params.id
        ...
        if(!(currentUser.bagasi.concat(currentUser.order).some(el => el == docID))) return next(new AppError('Dokumen tidak tersedia atau Kakak bukan pemilik dokumen ini 😢', 403));
        ...
        if(doc.every(el => el == null)) return next(new AppError('Dokumen yang dicari tidak tersedia 😢', 401));
        ...
        const newDokumen = await Model.findByIdAndUpdate(docID, {
        ...
        if(!newDokumen) return next(new AppError('Update Dokumen tidak berhasil 😢', 401));
    };
    
    res.status(200).json({
        status: 'Success',
        message: 'Dokumen berhasil di upload. Admin akan memeriksa validitas dokumen',
        dokumen: updateDokumen
    });
});
