...
exports.masuk = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password)
    return next(new AppError("Masuk/Login dengan Email dan Password", 400));

  const user = await User.findOne({ email }).select("+password");

  if (!user || !(await user.checkPassword(password, user.password)))
    return next(new AppError("Email atau Password Kakak salah", 401)); //* user.checkPassword adalah method yang sy buat sendiri di userModel
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.checkPassword(req.body.passwordLama, user.password)))
    return next(new AppError("Password lama Kakak salah 😢", 401));
...
exports.lupaPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user)
    return next(new AppError("Email belum ada dalam database kami", 404));
  ...
  try {
    ...
    return next(
      new AppError(
        "Internal Error, hubungi admin kami melalui email atau WhatsApp",
        500
      )
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const hashedToken = crypto
    .createHash("sha256")
    .update(resetToken)
    .digest("hex"); //! Random token dr client di hashed kemudian di compare dlm db
  ...
  if (!user)
    return next(new AppError("Invalid Token, hubungi admin kami", 400));
...
exports.googleOauthHandler = catchAsync(async (req, res, next) => {
  //* This fn handle this callback url www.nama.com/oauth/google
  ...
  if (!googleUser.verified_email)
    return next(
      new AppError(
        "Email kakak belum di verifikasi oleh Google. Verifikasi dulu ya kak 🙂",
        401
      )
    );
  ...
  if (!newGoogleUser || !copyUserGoogleToUser || !newUser)
    return next(new AppError("Kesalahan dalam mendaftar", 400));
...
exports.authGoogleHandler = passport.authenticate("google", {
  scope: ["profile", "email"],
});

//* www.domain.com/auth/google/redirect
(exports.authGoogleHandlerRedirect = passport.authenticate("google", {
  failureRedirect: "/",
})),
  (req, res) => {
    res.redirect("/bagasi");
...
exports.authFacebookHandler = passport.authenticate("facebook", {
